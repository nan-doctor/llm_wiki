...
- [ ] 获得外部写入授权与有效 GitHub 登录后，只提交本工具、计划、CI 和精确 `.gitignore` 变更，等待 Linux/Windows runner 实际结果。
- [ ] 在外部条件未满足时保持 Goal 活跃并准确报告阻塞，不用本机测试冒充远端结果。
...
- 发布判断：仍保持 `0.1.0` 候选状态。解除条件是当前 Codex Goal 运行时可用后补做不含第二 turn 的 Goal 验证，以及获得授权和有效 GitHub 登录后取得 Linux、Windows Actions 实际通过结果。
