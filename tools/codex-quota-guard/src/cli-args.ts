export type ParsedCliArgs =
  ...
  | { command: "doctor"; json: boolean; liveCanary: boolean }
  | {
      command: "run"
      prompt: string
  ...
  | { command: "resume"; prompt: string | undefined; json: boolean }
...
export function parseCliArgs(args: string[]): ParsedCliArgs {
  ...
  if (command === "status") {
    const parsed = parseOptions(rest, new Set(["json"]))
    if (parsed.positionals.length > 0) throw new Error(`${command} 不接受位置参数`)
    return { command, json: parsed.flags.has("json") }
  }
  if (command === "doctor") {
    const parsed = parseOptions(rest, new Set(["json", "live-canary"]))
    if (parsed.positionals.length > 0) throw new Error(`${command} 不接受位置参数`)
    return {
      command,
  ...
  if (command === "resume") {
    const parsed = parseOptions(rest, new Set(["json"]))
    const prompt = parsed.positionals.join(" ").trim()
    return { command, prompt: prompt || undefined, json: parsed.flags.has("json") }
  }
...
function parseOptions(args: string[], allowed: Set<string>): {
  ...
  const booleanFlags = new Set(["json", "require-protection", "live-canary"])
