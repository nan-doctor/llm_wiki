...
export async function runLiveCanary(client: DoctorRequestClient): Promise<LiveCanaryExecution> {
  ...
  try {
    ...
    const interruptedTurn = await startAndInterruptCanaryTurn(client, threadId, (startedId) => {
      turnId = startedId
    })
  ...
  const succeeded = runtime.turnStart === true
    ...
    && errors.length === 0
  return {
    result: { attempted: true, threadId, turnId, succeeded, errors },
    runtime,
  }
}
